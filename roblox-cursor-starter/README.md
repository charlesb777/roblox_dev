...
## Bonus superpower: what's an MCP? 🛰️

**MCP** (Model Context Protocol) is like a **walkie-talkie between your AI and an app.** The
**Roblox Studio MCP** lets Cursor's AI look *inside your open Roblox game* — every part, folder,
and script in the live game — and read them (or edit them, if you allow it) without copy-pasting.

Here's how it differs from Rojo:

| Tool | Direction | What it does |
|------|-----------|--------------|
| **Rojo** | your files → Studio | Pushes your `.luau` code files *into* Studio. |
| **Roblox Studio MCP** | Studio → Cursor | Lets Cursor *read the live game back out* — parts, models, UI, scripts. |

Why it matters: a lot of your game isn't code files — it's parts, models, and UI that live inside
the `.rbxl`. Rojo can't see those. **MCP lets Cursor see everything in the live game.** Together,
Rojo syncs your files *in* and MCP reads the game *out* — that's how you escape the "everything is
trapped inside the RBXL" problem.

### Your full stack
- **🛰️ Roblox Studio MCP** — Cursor inspects/edits the live `.rbxl` DataModel (Studio → Cursor)
- **🔗 Rojo** — Cursor/Git manage your Luau files and sync them in (files → Studio)
- **🏗️ Roblox Studio** — map, models, assets, UI, testing, publishing
- **🗂️ Git** — save, undo, back up (optional)

Turn the MCP on in **Step 7** below.
...
# Step 7 — Enable the Roblox Studio MCP (the superpower) 🛰️

This is Roblox's **official, built-in** MCP server. It lets Cursor read (and, if you allow it, edit)
your **live** game — the parts and scripts already inside Studio. Prefer this over random third-party
MCP plugins.

### 7a. Turn it on in Studio
1. Open Roblox Studio and open your place. **Keep Studio open the whole time** — MCP talks to the
   *open* session, not the `.rbxl` on disk.
2. Open **Assistant Settings → MCP Servers**.
3. Toggle **Enable Studio as MCP server** ON. When a client connects it shows **"● 1 client connected."**
4. Under **Quick connect**, flip **Cursor** ON. (Antigravity, Claude Desktop, Codex, and VS Code are also listed.)

### 7b. Restart Cursor and verify
1. Restart Cursor.
2. Go to **Cursor → Settings → Tools & MCPs**.
3. Under **Installed MCP Servers** you should see **Roblox_Studio** (e.g. *27 tools enabled*), toggled on.
   If it's off, toggle it on.

> **Manual fallback:** if Quick Connect doesn't take, Studio's **MCP Servers** screen shows a manual
> JSON config. In Cursor: **Settings → Tools & MCPs → New MCP Server** (this opens `~/.cursor/mcp.json`),
> and paste the exact `Roblox_Studio` JSON that Studio shows you — not one from a random blog.

### 7c. Test it (read-only)
Paste this into Cursor's Agent chat. Approve the MCP tool calls when Cursor asks:

```
Use the Roblox Studio MCP in READ-ONLY mode. Do not modify anything.
Read the current game tree and list the children of Workspace,
ServerScriptService, ReplicatedStorage, and StarterPlayerScripts,
plus every Script / LocalScript / ModuleScript with its full path.
```

A good result lists your Workspace (Camera, Terrain, Baseplate, SpawnLocation…) and every script path.

### ⚠️ Safety
MCP can change your live game. Start every session by telling Cursor:

```
Do not modify anything unless I explicitly say "make changes".
```

Once you trust it, you can ask it to read your Studio-made ModuleScripts and **move them into your
Rojo `src/` folders — but keep the Studio copies until you confirm.**

---

# Step 8 — Let Cursor's AI reverse-engineer your Rojo setup
...
| Cursor doesn't see `Roblox_Studio` MCP | MCP off, or Cursor not restarted | Enable Studio MCP + Quick-connect Cursor (Step 7), restart Cursor. |
| MCP returns no game data | Studio closed / wrong place open | Keep the place open — MCP reads the live session. |
